...
// Функция проверяет, совпадают ли параметры с одной из конкретных кривых
...
// Функция, которая преобразует хеш в большое целое число
fn hash_to_int(hash: &[u8], c: &Curve) -> BigInt {
    let order_bits = c.params().n.bits(); // Извлекаем битовую длину порядка
    let order_bytes = (order_bits + 7) / 8; // Определяем количество байт
    let mut hash = hash.to_vec();

    if hash.len() > order_bytes as usize {
        hash.truncate(order_bytes as usize); // Обрезаем хеш до нужных байт
    }

    let mut ret = BigInt::from_bytes_be(num_bigint::Sign::Plus, &hash); // Преобразуем в BigInt
    let excess = hash.len()*8 - order_bits; // Вычисляем избыток битов
    if excess > 0 {
        ret >>= excess; // Сдвигаем вправо
    }
    ret
}
...
pub fn verify_nistec(pub_key: &PublicKey, hash: &[u8], r: &BigInt, s: &BigInt) -> bool {
    ...
    let (x1, y1) = c.scalar_base_mult(&u1.to_bytes_be().1); // Получаем x1 и y1
    let (x2, y2) = c.scalar_mult(&pub_key.x, &pub_key.y, &u2.to_bytes_be().1); // Получаем x2 и y2
    let (x, y) = c.add(&x1, &y1, &x2, &y2); // Складываем точки

    if x.is_zero() && y.is_zero() { // Проверяем, не является ли точка бесконечностью
        return false;
    }

    let x_final = x.modpow(&BigInt::one(), n); // x = x mod N
    x_final == *r // Сравниваем x с r
}
...
pub fn mod_inverse(g: &BigInt, n: &BigInt) -> Option<BigInt> {
    ...
    // GCD ожидает, что параметры a и b будут > 0.
    if n.sign()==Sign::Minus {
        n = n.abs()//n.neg = false; // Преобразуем n в положительное
    }
...
pub fn gcd(a: &BigInt, b: &BigInt) -> (BigInt, BigInt) { // pub fn gcd(a: &BigInt, b: &BigInt) -> (BigInt, BigInt, BigInt) {
    ...
    while b != BigInt::zero() {
        let (q, r) = a.div_rem(&b); // Делим a на b, получаем частное и остаток
